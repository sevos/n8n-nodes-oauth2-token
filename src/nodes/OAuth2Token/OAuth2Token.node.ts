...
export class OAuth2Token implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		LoggerProxy.debug('OAuth2Token: Retrieved credentials, checking for access token');
		...
		if (oAuth2ApiCredentials.accessToken) {
			accessToken = oAuth2ApiCredentials.accessToken as string;
		} else if (oAuth2ApiCredentials.access_token) {
			accessToken = oAuth2ApiCredentials.access_token as string;
		} else if (oAuth2ApiCredentials.token) {
			accessToken = oAuth2ApiCredentials.token as string;
		} else if (oAuth2ApiCredentials.oauth && (oAuth2ApiCredentials.oauth as IDataObject).access_token) {
			accessToken = (oAuth2ApiCredentials.oauth as IDataObject).access_token as string;
		} else if (oAuth2ApiCredentials.data && (oAuth2ApiCredentials.data as IDataObject).access_token) {
			accessToken = (oAuth2ApiCredentials.data as IDataObject).access_token as string;
		} else if (oAuth2ApiCredentials.oauthTokenData) {
			// Some implementations store it in oauthTokenData
			const tokenData = oAuth2ApiCredentials.oauthTokenData as IDataObject;
			accessToken = (tokenData.access_token || tokenData.accessToken) as string;
		}
		
		// Log credential structure for debugging (securely)
		LoggerProxy.debug('OAuth2Token: Credential structure', {
		...
		if (!accessToken) {
			throw new NodeOperationError(this.getNode(), 'No access token available in the OAuth2 credentials. Please check your OAuth2 configuration and ensure the credentials are properly authorized. The credential data does not contain an accessToken, access_token, or token property.');
		}
		
		// Log access token for debugging (securely)
		LoggerProxy.debug('OAuth2Token: Successfully retrieved access token', {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				LoggerProxy.debug('OAuth2Token: Current item.json state', { 
					itemIndex,
